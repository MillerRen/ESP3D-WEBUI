...
StatusExtractor.prototype.statusReport = function (message) {
  ...
  if (message.includes(',MPos:') || message.includes(',WPos:')) {
    ...
    statusParams.forEach(function (param) {
      ...
      if (key === 'Buf') buffer[0] = value.replace(',', '')
      else if (key === 'RX') buffer[1] = value.replace(',', '')
      else report[statusMap[key]] = value
    })

    if (buffer.length > 0) report.buffer = buffer.join(',')
  } else {
...
StatusExtractor.prototype.parsePins = function (pins) {
  ...
  if (/([01]+),?([01])?,?([01]+)?/.test(pins)) {
    ...
    pinData.forEach(function (pin) {
      data.push({ pin: grbl11pinMap[pin], on: true })
    })
  }
  return data
}
...
StatusExtractor.prototype.parseAccessories = function (accessories) {
  ...
  if (flags.indexOf('F') > -1) parsedAccessories.flood = true
  else parsedAccessories.flood = false

  if (flags.indexOf('M') > -1) parsedAccessories.mist = true
  else parsedAccessories.mist = false

  if (flags.indexOf('S') > -1) parsedAccessories.spindleDirection = 'clockwise'
  else parsedAccessories.spindleDirection = 'counter-clockwise'
